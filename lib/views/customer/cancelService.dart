...
class _CancelServicePageState extends State<CancelServicePage> {
  ...
  List<ReserveDetail>? _listReserveDetails;
  ...
  void fetchData() async {
    ...
    _listReserve = await _reserveController.listReserves("ongoing", userId!);
    for (var item in _listReserve!) {
      scheduleDate = item.scheduleDate.toString().split(" ")[0];
      reserveId = item.reserveId;
      _barberName =
          "${item.barberModel?.userModel?.firstName} ${item.barberModel?.userModel?.lastName}";
      _totalPrice = item.price;
    }
    _listReserveDetails =
        await _reserveDetailController.listReserveDetails(reserveId!);

    setState(() {
      isLoaded = true;
    });

    heightScore = 70.0 * _listReserveDetails!.length;
  }
                              ...
                              showDialog(
                                  ...
                                  builder: (BuildContext context) =>
                                      AlertDialog(
                                        ...
                                        content: Text(
                                            'ท่านต้องการยกเลิกการจองหรือไม่'),
                                        actions: <Widget>[
                                          TextButton(
          ...
          children: [
            ...
            Container(
              ...
              child: ListView.builder(
                  itemCount: _listReserveDetails?.length,
                  itemBuilder: ((context, index) {
                    String formattedTime = '';
                    if (_listReserveDetails?[index].scheduleTime != null) {
                      try {
                        DateTime parsedTime = DateTime.parse(
                            "${_listReserveDetails?[index].scheduleTime!}");
                    ...
                    return ListTile(
                      title: Text(
                          "${_listReserveDetails?[index].service?.serviceName}   "
                          "${_listReserveDetails?[index].service?.price}   บาท    "
                          "เวลา ${formattedTime}"),
                    );
                  })),
            ),
            Text(" ช่างตัดผม:  ${_barberName}"),

            Text(" ราคารวม:  ${_totalPrice}"),
                  ...
                  children: [
                    Padding(
                      ...
                      child: ElevatedButton(
                          ...
                          onPressed: () {
                            _showSureToDeleteReserveAlert(reserveId!);
                          },
                          child: const Text(
                            'ยกเลิกการจอง',
